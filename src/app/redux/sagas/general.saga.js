...
function* updateEntity(action) {
  ...
  try {
    ...
    if (config.routeAfter) {
      const id = data.id;
      const link = generateRoute(config.routeAfter.name, {
        [config.routeAfter.param]: id
      });
...
function* deleteFile(action) {
  ...
  try {
    ...
    if (config.afterFileDeletedAction) {
      yield put(
        config.afterFileDeletedAction(config.afterFileDeletedActionParams)
      );
    }
...
function* generalRequest(action) {
  ...
  try {
    ...
    if (redirectTo) {
      entityUpdated(
        config.message ? config.message : "Request successful",
        redirectTo
      );
    } else {
