...
class MaterialForm extends Component {
    ...
    render() {
        ...
        return (
            <tr>
                <td>
                    <input type="text" id="table-item" onChange={this.handleChange.bind(null, 'name')}
                           value={name.value}
                           className={'md-input ' + name.error}/>
                </td>
                <td>
                    <input type="text" id="table-item" onChange={this.handleChange.bind(null, 'quantity')}
                           value={quantity.value} className={'md-input ' + quantity.error}/>
                </td>
                <td>
                    <select onChange={this.handleChange.bind(null, 'unit')} value={unit.value} id="table-item"
                            className={'md-input ' + unit.error}>
                        <option value="">Select...</option>
                        {this.props.options.map((option, i) => {
                            return <option key={i} value={option.id}>{option.shortcut}</option>
                        })}
                    </select>
                </td>
                <td>
                    <input type="text" id="table-item" onChange={this.handleChange.bind(null, 'price')}
                           value={price.value}
                           className={'md-input ' + price.error}/>
                </td>
                <td>
                    <div className="uk-width-medium-1-6">
                        <a id="table-item" onClick={this.handleSubmit}
