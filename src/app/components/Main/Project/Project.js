...
class Project extends Component {
  ...
  isDirty = () => {
    ...
    return (
      this.state.formsDirty.ProjectForm || this.state.formsDirty.ProjectAclForm
    );
  };
  ...
  dispatchIsDirty = (formName, dirty, pristine) => {
    ...
    this.setState({
      formsDirty: Object.assign({}, this.state.formsDirty, {
        [formName]: dirty
      })
    });
  ...
  userAddOnSubmit = data => {
    let config = configResolver.projectUserConfig(
      this.props.params.projectId,
      data.userId
    );
  ...
  projectAclOnSubmit = values => {
    ...
    const configAcl = configResolver.projectAclConfig(
      this.props.params.projectId
    );
    let newValues = {};
  ...
  removeUser = (id, e) => {
    e.preventDefault();
    let config = configResolver.projectUserDeleteConfig(
      this.props.params.projectId,
      id
    );
  ...
  render() {
    ...
    return (
      <div>
        <ProjectForm
          formDirty={this.state.formsDirty.ProjectForm}
          dispatchIsDirty={this.dispatchIsDirty}
          onSubmit={this.onSubmit}
          {...this.props}
          heading={this.props.project ? "Edit project" : "Add project"}
        />

        {this.props.project &&
          this.props.project.canEdit && (
            <UserAddForm onSubmit={this.userAddOnSubmit} {...this.props} />
          )}
        {this.props.project &&
          this.props.project.canEdit && (
            <ProjectAclForm
              onSubmit={this.projectAclOnSubmit}
              {...this.props}
              formDirty={this.state.formsDirty.ProjectAclForm}
              dispatchIsDirty={this.dispatchIsDirty}
...
function mapStateToProps(state, ownProps) {
  ...
  const project = state.system.menu.projects.filter(
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(
      { ...actions, ...generalActions, ...systemActions },
      dispatch
    )
  };
}
