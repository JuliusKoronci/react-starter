...
class User extends Component {
  constructor(props, context) {
    ...
    this.profileAvatarConfig = configResolver.getProfileAvatarConfig(
      props.params.userId
    );
  }
  ...
  submitHandler = values => {
    let userAdditionalConfig = configResolver.getUserAdditionalConfig(
      this.props.params.userId,
      values.user_role.id,
      values.company.id,
      !!this.props.user
    ...
    this.userConfig.remapValues = Object.assign(
      this.userConfig.remapValues,
      userAdditionalConfig.additionalFields
    );
    // console.log(this.userConfig.remapValues);

    if (this.props.params.userId) {
      let config = configResolver.userUpdate(
        this.props.params.userId,
        values.user_role.id,
        values.company.id
      );
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(
      { ...actions, ...generalActions, ...userActions },
      dispatch
    )
  };
}
