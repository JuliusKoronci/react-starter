...
class Imap extends Component {
  ...
  onSubmit = values => {
    ...
    if (this.props.params.imapId) {
      let config = configResolver.imapUpdate(
        this.props.params.imapId,
        values.project.id
      );
  ...
  render() {
    return (
      <View
        ...
        {...this.props}
        heading={this.props.imap ? "Edit imap" : "Add imap"}
...
function mapStateToProps(state, ownProps) {
  ...
  const imap = state.imaps.data.filter(
    imap => parseInt(imap.id, 10) === parseInt(imapId, 10)
  );
  // const projects = state.projects.data;
  const projects = state.system.menu.projects.filter(
