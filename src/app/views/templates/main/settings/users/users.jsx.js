import React, {PropTypes} from 'react';
import {Link} from 'react-router';

const users = (prop) => {
    return (
        <div className="md-card">
            <div className="md-card-content">
                <div className="uk-margin-bottom" data-uk-margin>
                    <h1 className="heading_b uk-margin-bottom">Users</h1>
                </div>
                <hr/>
                <div className="uk-input-group">
                    <span className="uk-input-group-addon"><i className="material-icons md-24">&#xE8B6;</i></span>
                    <input type="text" className="md-input" />
                    <span className="uk-input-group-addon"><a className="md-btn md-btn-primary" href="#">Search</a></span>
                </div>

                <table className="uk-table uk-text-nowrap">
                    <thead>
                    <tr>
                        <th>Name</th>
                        <th className="uk-text-center">Email</th>
                        <th className="uk-text-center">Company</th>
                        <th className="uk-text-center">Active</th>
                        <th className="uk-text-center">Action</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                        <td>Branislav Susta</td>
                        <td className="uk-text-center">branislav.susta@gmail.com</td>
                        <td className="uk-text-center">Company 1</td>
                        <td className="uk-text-center">Yes</td>
                        <td className="uk-text-center">
                            <a className="md-btn md-btn-danger" href="#">Delete</a>
                            <a className="md-btn md-btn-primary" href="settings_user.html">Edit</a>
                        </td>
                    </tr>
                    <tr>
                        <td>Julius Krivda</td>
                        <td className="uk-text-center">julius.krivda@gmail.com</td>
                        <td className="uk-text-center">Company 1</td>
                        <td className="uk-text-center">Yes</td>
                        <td className="uk-text-center">
                            <a className="md-btn md-btn-danger" href="#">Delete</a>
                            <a className="md-btn md-btn-primary" href="/settings/users/user">Edit</a>
                        </td>
                    </tr>
                    </tbody>
                </table>
                <div className="text-allign-right">
                    <Link to="/settings/users/add" className="md-btn md-btn-primary" >Add</Link>
                </div>
            </div>
        </div>
    );
};

users.propTypes = {
    prop: PropTypes.object.isRequired
};

export default users;
