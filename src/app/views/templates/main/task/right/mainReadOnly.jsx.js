...
const main = ({task, handleFileDownload, form}) => {
    ...
    if (task.taskHasAssignedUsers.length > 0) {
        ...
        status = <div style={{
            background: task.taskHasAssignedUsers[0].status.color,
            color: 'white',
            padding: '3px 8px'
        }}>{task.taskHasAssignedUsers[0].status.title}</div>;
    }
        ...
        <div className="md-list md-list-addon">
            ...
            <div className="uk-input-group" style={{marginTop: '20px'}}>
                ...
                <div
                    ...
                    {form.tags.map(tag => {
                        return <span
                            style={{background: '#' + tag.color, marginRight: '8px', color: 'white', padding: '3px'}}
                            key={tag.id}>{tag.title}</span>
                    })}
                </div>
            </div>
