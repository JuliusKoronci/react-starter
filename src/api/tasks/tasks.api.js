...
function loadDefault(filterId, search) {
  let config = {
    method: "GET",
    headers: {
  ...
  return fetch(url, config)
    .then(response => response.json().then(tasks => ({ tasks, response })))
    .then(({ tasks, response }) => {
      if (!response.ok) {
...
export function defaultFilter(filterId, search) {
  if (USE_MOCK) {
    return mockDefault();
  }
  return loadDefault(filterId, search);
}
...
//
// export function updateWholeTask(taskId, data) {
//     let config = {
//         method: 'PATCH',
//         body: queryString.stringify(data),
//         headers: {
